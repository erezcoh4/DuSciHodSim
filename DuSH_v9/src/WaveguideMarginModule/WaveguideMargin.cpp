...
// ------------------------------------------------------- //
WaveguideMargin::WaveguideMargin (const char *name, Double_t dx, Double_t dy, Double_t dz, Double_t *origin):
...
// ------------------------------------------------------- //
void WaveguideMargin::defineFacets(){
...
// ------------------------------------------------------- //
void WaveguideMargin::SetTotalInternalReflectionAngle(){
    // with respect to air
...
// ------------------------------------------------------- //
bool WaveguideMargin::ContainsPoint(const TVector3 position) {
...
//WaveguideMargin::WaveguideMargin (const char *name, Double_t fdx1, Double_t fdx2, Double_t fdy1, Double_t fdy2, Double_t fdz):
//TGeoTrd2 (name, fdx1, fdx2, fdy1, fdy2, fdz){
//    std::cout << "constructed WaveguideMargin" << std::endl;
//    dz = fdz;
//    dx1 = fdx1;
//    dx2 = fdx2;
//    dy1 = fdy1;
...
//
//    if (verbose>2){
//        Debug(2, "WaveguideMargin::defineFacets()");
//        for (auto facetCenter : facetCenters){
//            PrintTVector3( facetCenter );
//        }
//        for (auto facetNormal : facetNormals){
...
//
//// ------------------------------------------------------- //
//bool WaveguideMargin::CheckIfPointOnFacet ( int facetIdx, TVector3 pt ){
//    Debug(2,"WaveguideMargin::CheckIfPointOnFacet()");
...
//
//// ------------------------------------------------------- //
//bool WaveguideMargin::ContainsPoint(const TVector3 pt) {
//    //    Double_t point[3] = {position.x(), position.y(), position.z()};
//    //    return this->Contains(point);
//    double fdz = dz;
//    double fdx1 = dx1, fdx2 = dx2, fdy1 = dy1, fdy2 = dy2 ;
//
//    double margin = 0.1;
//    if (TMath::Abs(pt.z()) > fdz) {
//        Debug(2,Form("|pt.z=%.4f| > fdz=%.1f)",pt.z(),fdz));
//        return false;
//    }
//    Double_t fdy = 0.5*(fdy2*(pt.z()+fdz) + fdy1*(fdz-pt.z()))/fdz;
//    if (TMath::Abs(pt.y()) > fdy + margin) {
//        Debug(2,Form("|pt.y=%.4f| > fdy=%.4f)",pt.y(),fdy));
//        return false;
//    }
//
//    Double_t fdx = 0.5*(fdx2*(pt.z()+fdz) + fdx1*(fdz-pt.z()))/fdz;
//    if (TMath::Abs(pt.x()) > fdx + margin) {
//        Debug(2,Form("|pt.x=%.4f| > fdx = %.4f)",pt.x(),fdx));
//        return false;
//    }
//    return true;
//}
